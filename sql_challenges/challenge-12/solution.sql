...
-- 1. What is the business question?
    -- compare how quickly each team completes work relative to other teams
-- 2. What is the exact definition? (Include every filter, every join)
    -- Velocity = completed tasks per team member per day
        COUNT(completed_tasks)
        / number_of_days_in_period
-- 3. What are the edge cases? (NULLs, cancelled tasks, unassigned tasks, etc.)
    -- teams with 0 completed tasks, task complexity
-- 4. What is the unit? (Count, percentage, hours, dollars?)
...
WITH task_summary AS (
    SELECT
        ...
        MAX(TRUNC(t.completed_at)) - MIN(TRUNC(t.completed_at)) + 1 AS total_days
    FROM tasks t
    LEFT JOIN users u
...
SELECT
    ...
    ROUND(
        completed_tasks / NULLIF(total_days, 0),
        2
    ) AS velocity,
...
SELECT
    ...
    CASE
        WHEN COUNT(*) = 1
        THEN 'Only 1 completed task — average may not be meaningful'
        WHEN COUNT(*) < 5
        THEN 'Low sample size'
        ELSE 'Sample size acceptable'
